...
import { useTaglistsContext } from '../../hooks/useTaglistContext';
import { useAuthContext } from '../../hooks/useAuthContext';
import { useProjectsContext } from '../../hooks/useProjectsContext';

const Taglists = () => {
  const { taglists, dispatchTaglists } = useTaglistsContext();
  const { user } = useAuthContext();
  const projects = useProjectsContext;
  const removeTaglistHandle = async (project) => {
    const response = await fetch(`api/taglists/${project._id}`, {
      method: 'DELETE',
      headers: {
    ...
    if (response.ok) {
      ...
      dispatchTaglists({ type: 'DELETE_PROJECT', payload: json });
    }
  };
  ...
  return (
    <div className="taglists-list-wrap">
        ...
        {projects &&
          projects.map((project) => (
            <div className="project" key={project._id}>
              ...
              <span onClick={() => removeTaglistHandle(project)} className="material-icons remove-project-button">
                close
              </span>
            </div>
          ))}
